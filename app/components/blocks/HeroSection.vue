...
onMounted(() => {
  ...
  tl.from(badge.value,  { y: 16, opacity: 0, duration: 0.6 })
    ...
    .from(card.value,   { y: 40, opacity: 0, duration: 1, ease: 'power4.out' }, '-=0.45')
...
<template>
  <section class="relative min-h-screen flex flex-col items-center justify-center pt-14 overflow-hidden">
    <!-- Grid background -->
    <div class="absolute inset-0 bg-[linear-gradient(to_right,#ffffff06_1px,transparent_1px),linear-gradient(to_bottom,#ffffff06_1px,transparent_1px)] bg-[size:64px_64px]" />
    <!-- Glow -->
    ...
    <div class="relative z-10 flex flex-col items-center text-center px-4 max-w-5xl mx-auto w-full">
      ...
      <h1 class="font-bold tracking-tight leading-none mb-6">
        <span ref="line1" class="block text-6xl md:text-8xl text-white">Don't install.</span>
        <span ref="line2" class="block text-6xl md:text-8xl text-zinc-600">Just copy.</span>
      </h1>

      <!-- Description -->
      <p ref="desc" class="text-lg text-zinc-400 max-w-lg mx-auto mb-10 leading-relaxed">
        Production-ready Nuxt 4 components. TypeScript, Tailwind, GSAP micro-interactions — grab the code and ship.
      </p>

      <!-- CTAs -->
      <div ref="ctas" class="flex items-center justify-center gap-3 flex-wrap mb-24">
