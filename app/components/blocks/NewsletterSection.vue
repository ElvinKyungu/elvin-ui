...
<template>
  <section ref="section" class="py-24 px-6 bg-zinc-950 relative overflow-hidden">
    ...
    <div class="max-w-2xl mx-auto flex flex-col items-center gap-8 text-center relative z-10">
      ...
      >
        <div v-if="status !== 'success'" key="form" class="nl-enter w-full flex flex-col gap-3 sm:flex-row sm:gap-2">
        ...
        <div v-else key="success" class="nl-enter flex items-center gap-3 px-6 py-4 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-400">
      ...
      >
        <p v-if="status === 'error' && errorMsg" class="text-xs text-red-400 -mt-5">{{ errorMsg }}</p>
      </Transition>
